        ...
        {isLoading || detailLoading ? (
          ...
          <div className="rounded-lg border overflow-hidden">
            ...
            <div className="grid grid-cols-12 gap-2 px-4 py-2 bg-muted text-xs font-medium text-muted-foreground">
              <div className="col-span-3">User ID / Name</div>
            ...
            <div className="divide-y max-h-96 overflow-y-auto">
              {profiles.map((p: any) => (
                <div
                  key={p.id}
                  className="grid grid-cols-12 gap-2 px-4 py-3 text-sm items-start hover:bg-muted/50 cursor-pointer"
                  onClick={() => handleViewProfile(p.id)}
                >
                  <div className="col-span-3 truncate">
                    <div className="font-medium truncate">{p.user_id || p.id}</div>
                    <div className="text-xs text-muted-foreground truncate">{p.name || <span className="italic">(no name)</span>}</div>
                  </div>
...
function AIVerificationTable({ profile, vh, server, apiKey, onUpdated }: {
  ...
  const rows = [
    ...
    { field: 'IP Address', key: '', submitted: profile.api_submitted_ip_address || '', extracted: vh.ip_address || '' },
  ]
